...
object GeoJsonLoader {
    fun loadGeoJsonFeatures(onResult: (List<RegionFeature>) -> Unit) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val url =
                    "https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Counties_and_Unitary_Authorities_December_2024_Boundaries_UK_BSC/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson"
                ...
                for (i in 0 until features.length()) {
                    try {
                        ...
                        val name = props.getString("CTYUA24NM")
                        ...
                        featureList.add(RegionFeature(name, polygon))
                    } catch (e: Exception) {
